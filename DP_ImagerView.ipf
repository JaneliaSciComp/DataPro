...
Function ImagerViewUpdate()
	...
	Variable epiTTLOutputIndex=EpiLightGetTTLOutputIndex()	
	Variable isTriggerTTLAvailable=( !SweeperGetTTLOutputChannelOn(triggerTTLOutputIndex) && !TestPulserIsTTLInUse(triggerTTLOutputIndex) && !EpiLightIsTTLOutputInUse(triggerTTLOutputIndex) )
	Variable isEpiTTLAvailable=!SweeperGetTTLOutputChannelOn(epiTTLOutputIndex)
	Variable exposureADCIndex=ImagerGetExposureADCIndex()
	Variable isExposureADCAvailable=( !SweeperGetADCOn(exposureADCIndex) && !TestPulserIsADCInUse(exposureADCIndex) )	
	Variable isTriggeredCBEnabled=(isTriggered || (isTriggerTTLAvailable && isExposureADCAvailable && isEpiTTLAvailable) )
	CheckBox isTriggeredCB, win=ImagerView, value=isTriggered, disable=fromEnable(isTriggeredCBEnabled)
